...
function setupIntroCarousel() {
  ...
  Promise.all(candidates.map(probe)).then(results => {
    ...
    // Touch + mouse drag. Vertical movement is left to the page so
    // normal scrolling still works.
    shell.style.touchAction = "pan-y";

    shell.addEventListener("pointerdown", event => {
      if (event.target.closest(".intro-dot")) return;
    ...
    shell.addEventListener("pointerup", event => {
      ...
      if (valid.length > 1 && moved && Math.abs(dx) >= 45 && Math.abs(dx) > Math.abs(dy)) {
